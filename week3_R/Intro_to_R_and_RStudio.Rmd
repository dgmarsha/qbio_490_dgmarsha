...
```{r setup Exercise 2.3}
    ...
    knitr::opts_knit$set(root.dir = normalizePath("C:/Users/ME/Documents/QBIO/sp24_course_work_490/qbio_490_dgmarsha/week3_R")) 
```
...
age = 47 #age is a numeric.
print(typeof(age))

birthday = "01/29/2024" #birthday is a string.
print(typeof(birthday))

three_fav_colors = c("Green", "Blue", "Indigo") #three_fav_colors is a vector of strings.
print(typeof(three_fav_colors))

#These don't match. The fxn class() gives the answers I provided. Their types are apparently all character but for the second, a double.
```
...
```{r Exercise 5.2}
"cat" == "cat" & "dog" == "dog" # prediction: T
"cat" == "cat" & "fish" == "hamster" # prediction: F

1 < 2 | 3 < 1 # prediction: T
1 < 2 | 3 < 5 # prediction: T
...
```
  ...
  Sometimes using "if... else if... else" doesn't work. You might get the error code: "the condition has length > 1". This is because an if statement only checks ONE element to evaluate the condition. To performs an element-wise evaluation on a vector, we must use ifelse(), a vectorized function. The syntax is as follows:
...
```{r Exercise 6.8}
# write loop here   - I am assuming inclusive.
for (i in -7:7) {
  # do something
  print(i)
}
...
Create a new column called "transmission" that assigns the value  where am = 0, and "manual" where am = 1.
