...
```{r}
write.csv(clinic, "C:/Users/ME/Documents/QBIO/sp24_cw/qbio_490_dgmarsha/analysis_data/skcm_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("C:/Users/ME/Documents/QBIO/sp24_cw/qbio_490_dgmarsha/analysis_data/skcm_clinical_data.csv")
```
...
str(clinic)
head(clinic) 
#head for just the first few lines, str for the structure.

# 1. r: 470 c:65
# 2. Data type of $gender: chr
# 3. What is the age in years of the 5th patient? She was alive as of last contact,   in what seems to have been in 2013 if that is what form completion date means. I have not found an explanation online. She was 74 as of her diagnosis in 2010,   so she is 88 if she is still around.  
# 4. How old was the 2nd patient at their last follow up? He was last contacted 1856 days from some date, maybe data of form completion in May of 2014. He was 82 at diagnosis, which may have been in 2009 based on initial_pathologic_dx_year. If last contact is the above, he was 92. If it means from diagnosis, then maybe 87.
```
...
```{r Exercise 4.1}
# create plot here
plot(x=clinic$weight_kg_at_diagnosis, y=clinic$breslow_thickness_at_diagnosis, main="Weight by Breslow Thickness at Diagnosis", xlab = "Weight (Kg)", ylab="Breslow Thickness (?)", col= blues9 )
```
...
```{r Exercise 4.2}
# create plot here
typeof(clinic$breslow_thickness_at_diagnosis)
#is chr?
typeof(clinic$weight_kg_at_diagnosis)
#Why is this not numeric?
boxplot(as.numeric(clinic$weight_kg_at_diagnosis) ~ clinic$vital_status, xlab = "Vital Status", ylab = "Weight (Kg)", main = "Vital Status Relation to Weight at Diagnosis")
#The whiskers are quartiles, not CI's.
```
...
```{r}
#clinic$gender = clinical.BCRtab.all$clinical_patient_skcm$gender[3:472]

#Accidentally set all to male by using only one = below.
#length(clinical.BCRtab.all$clinical_patient_skcm$gender[3:472])
male_mask <-clinic$gender == "MALE"
#male_mask
#clinical.BCRtab.all$clinical_patient_skcm$gender[471]
```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
under30_over60_mask <-(as.numeric(clinic$age_at_diagnosis) <35 | as.numeric(clinic$age_at_diagnosis)>60)
head(under30_over60_mask)
head(clinic$age_at_diagnosis)
```
...
```{r}
cleaned_clinic$age_at_diagnosis = as.numeric(cleaned_clinic$age_at_diagnosis)
cleaned_clinic$age_category <- ifelse(cleaned_clinic$age_at_diagnosis<=35,"Young", no = ifelse(cleaned_clinic$age_at_diagnosis>=50,"Old", "Middle" ))#It won't let me indent.
```
...
```{r}
cleaned_clinic$survival_time <- ifelse((cleaned_clinic$death_days_to!="[Not Available]" & cleaned_clinic$death_days_to!="[Not Applicable]"),
                                       cleaned_clinic$death_days_to,cleaned_clinic$last_contact_days_to )
cleaned_clinic$last_contact_days_to[310]
cleaned_clinic$death_days_to[310]
cleaned_clinic$survival_time
#There are still some NA's and Completions?
deep_cleaned_clinic <- cleaned_clinic[(cleaned_clinic$survival_time!="[Not Available]" & cleaned_clinic$survival_time!="[Not Applicable]" & cleaned_clinic$survival_time!="[Completed]"),]
deep_cleaned_clinic$survival_time
```
...
survival_object <- Surv(time = deep_cleaned_clinic$survival_time,
                        event = deep_cleaned_clinic$death_event)

fit_object <- survfit(survival_object ~ age_category, data = deep_cleaned_clinic)
```
...
# 1. What does the y axis represent? The odds a patient in that group is still alive.

# 2. What does the x axis represent? Days post diagnosis.

# 3. What age group appears to have the greatest rate of survival? The young.

# 4. What age group appears to have the worst rate of survival? The old.

# 5. What is the approx. 15-year survival rate for "Old" skin cancer patients? ~12%

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and skin cancer survival? Under .0001, which is quite significant and suggests that there is a meaningful relationship between age and survival.
