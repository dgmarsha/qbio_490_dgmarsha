...
#install.packages("maftools") #get this right.
library(maftools)
...
write.csv(clinical, "/PATH/TO/analysis_data/skcm_clinical_data.csv", row.names=FALSE)#perm change in colname
```
...
maf_object <- read.maf(maf = maf,
                       clinicalData = clinical, #can be anything with the tumour barcodes
                       isTCGA = TRUE)
```
...
tp53_mask<-ifelse(maf.data$Hugo_Symbol == 'TP53',T,F)
maf.data[tp53_mask, c('Tumor_Sample_Barcode', 'HG...)]
...
oncoplot(maf = maf_object,
         top = 10,
         borderCol = NA) 
#maf is the maf object with all the data
#top is n
...
oncoplot(maf = FILL IN HERE,
         ...
         borderCol = NA)
#can add clinical features oin bottom
```
...
#look at high riskand low risk group
#this is data represenation not stats, must raise own flag to be roughly similar sample sizes
...
```{r Exercise 9.2}
young_maf <- subsetMaf(maf = FILL IN HERE, #good fxn? Mind oncoplot only shows the patients with mutations in the columns.
                       tsb = FILL IN HERE)
#boolean Mf_clincal where maf young, has to be vector
#maf is maf object, tsb is vector of young barcodes
#
```
...
```{r Exercise 9.4}
coOncoplot(m1 = FILL IN HERE, #both mafs
           m2 = FILL IN HERE, 
           m1Name = FILL IN HERE, #titles
           m2Name = FILL IN HERE, 
           borderCol = NA)

ggsave("C:/Users/ME/Documents/QBIO/sp24_cw//qbio_490_NAME/coOncoplot_GENE_GENE_SKCM")
```   
...
#Then somatic interactions plot, type of heatmap
#n commonly mutated genes vs n commonly mutated genes
#looking for relations in mutation rate
#p labels. Can show likely together or separate. The following.

###### (6) Fisher's Exact Test - Demo
#gives p whetehr two vas likely to occur together
#compare odd ratios, odd event in treatment or control group comparsison. P val of that and odds ratio.
Rewrite the vital_status and gender columns of your clinical df as factors.

```{r Exercise 4.1}
clinical$vital_status <- clinic$vital_status
clinical$gender <- FILL IN HERE
```

Create a contingency table using table().

```{r Exercise 4.2}
contig <- table(clinic$vital_status[clinic$gender!="[Not Available]"],clinic$gender[clinic$gender!="[Not Available]"])
contig
```
...
#contingency table makes easier? or somatic interaction plot
...
somaticInteractions(maf = FILL IN HERE,
                    genes = FILL IN HERE, #can look at specifc or just teh top mutatted ones. top works better
...
somaticInteractions(maf = FILL IN HERE,
                    ...
                    pvalue = c(0.05, 0.1))
```
#Many are significantly co-occurrent.

###### (8) Lollipop Plots
#distribution of of mutations across gene, see domains for protein, see impact
...
ggsave("C:/Users/ME/Documents/QBIO/sp24_cw/qbio_490_dgmarsha/lollipop_TP53_SKCM")
```
...
```{r Exercise 10.3}
#can lollipop between groups of patients.
...
mafSurvival(maf = maf_object,
            ...
            time = "days_to_last_follow_up", ## name of the column in maf_object@clinical.data containing survival time
            #look at better way to predict based on columns
